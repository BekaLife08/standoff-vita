			...
			public void MoveNext()
			{
				...
				try
				{
					...
					catch (ConnectionFailedException)
					{
						...
						try { PhotonNetwork.playerName = newName; PlayerPrefs.SetString("PlayerName", newName); PlayerPrefs.Save(); } catch {}
						try { BoltService<BoltPlayerService>.Instance.Player.Name = newName; } catch {}
						_0024this.InternalSetPlayer();
					}
					catch (Exception ex2)
					{
						...
						try { PhotonNetwork.playerName = newName; PlayerPrefs.SetString("PlayerName", newName); PlayerPrefs.Save(); _0024this.InternalSetPlayer(); } catch {}
		...
		private struct _003CSetAvatarAsync_003Ec__async1 : IAsyncStateMachine
		{
			...
			public void MoveNext()
			{
				...
				try
				{
					...
					{
						Log.Debug(message);
						try { PhotonNetwork.player.SetAvatar(avatar); PlayerPrefs.SetString("PlayerAvatar", System.Convert.ToBase64String(avatar)); PlayerPrefs.Save(); } catch {}
						_0024this.InternalSetPlayer();
					}
					catch (Exception ex)
					{
						Log.Error(ex);
						try { PhotonNetwork.player.SetAvatar(avatar); PlayerPrefs.SetString("PlayerAvatar", System.Convert.ToBase64String(avatar)); PlayerPrefs.Save(); _0024this.InternalSetPlayer(); } catch {}
