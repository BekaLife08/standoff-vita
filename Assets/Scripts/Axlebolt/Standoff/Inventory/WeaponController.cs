...
namespace Axlebolt.Standoff.Inventory
{
	...
	public abstract class WeaponController : MonoBehaviour
	{
		...
		{
			ClearAudioSources();
			int shotCount = AnimationParameters.audioSourcesCount;
			if (shotCount <= 0) shotCount = 2;
			ShotAudioData.AudioSources = playerController.CharacterAudioPlayer.GetAudioSources(shotCount);
		...
		internal virtual void SetViewMode(ViewMode viewMode)
		{
			...
			case ViewMode.TPS:
				...
				{
					animationAudioSource2.spatialBlend = 0.7f;
					animationAudioSource2.maxDistance = 80f;
					animationAudioSource2.rolloffMode = AudioRolloffMode.Linear;
				}
				foreach (AudioSource audioSource2 in ShotAudioData.AudioSources)
				{
					audioSource2.spatialBlend = 0.7f;
					audioSource2.maxDistance = 80f;
