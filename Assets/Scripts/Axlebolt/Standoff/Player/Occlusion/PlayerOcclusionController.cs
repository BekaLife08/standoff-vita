...
namespace Axlebolt.Standoff.Player.Occlusion
{
	public class PlayerOcclusionController : ObjectOccludee
	{
		...
		public override List<Vector3> GetRaycastHitPoints(Vector3 casterPoint)
		{
			...
			list2[0] = list2[0] + vector2 * num;
			list2[1] = list2[1] + vector2 * (num * 0.5f);
			list2[2] = list2[2] + vector2 * (num * 0.5f);
			list2[3] = list2[3] + vector2 * ((0f - num) * 0.5f);
			list2[4] = list2[4] + vector2 * ((0f - num) * 0.5f);
			list2[5] = list2[5] + vector2 * (0f - num);
			return list;
		}
	}
}
